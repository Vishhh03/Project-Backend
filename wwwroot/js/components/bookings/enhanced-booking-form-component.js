/**
 * EnhancedBookingFormComponent - Booking form with loyalty points integration
 * Extends the basic booking form with loyalty rewards display and points calculation
 */
class EnhancedBookingFormComponent {
    ...
    render() {
        ...
        this.container.innerHTML = `
            <div class="booking-form-container enhanced">
                ...
                ${this.loyaltyAccount ? this.renderLoyaltyWidget() : ''}
                ...
                <form id="booking-form" class="booking-form">
                    ...
                    <div class="price-summary" id="price-summary" style="display: none;">
                        ...
                        ${this.loyaltyAccount ? this.renderLoyaltyEarnings() : ''}
        ...
        return `
            <div class="loyalty-widget">
                <div class="loyalty-header">
                    ...
                    <div class="membership-badge ${this.loyaltyAccount.membershipLevel.toLowerCase()}">
                        ${this.loyaltyAccount.membershipLevel}
                    </div>
                </div>
                <div class="loyalty-stats">
                    <div class="stat">
                        <span class="stat-value">${this.formatPoints(this.loyaltyAccount.pointsBalance)}</span>
                        <span class="stat-label">Available Points</span>
                    </div>
                    <div class="stat">
                        <span class="stat-value">${this.formatPoints(this.loyaltyAccount.totalPointsEarned)}</span>
    ...
    renderLoyaltyEarnings() {
        return `
            <div class="loyalty-earnings">
                <h5>🎉 Loyalty Rewards</h5>
                <div class="loyalty-summary">
                    <div class="loyalty-line">
                        <span class="loyalty-label">Current Points:</span>
                        <span class="loyalty-value">${this.formatPoints(this.loyaltyAccount.pointsBalance)}</span>
                    </div>
                    <div class="loyalty-line">
                        <span class="loyalty-label">Membership Level:</span>
                        <span class="loyalty-value membership-${this.loyaltyAccount.membershipLevel.toLowerCase()}">${this.loyaltyAccount.membershipLevel}</span>
                    </div>
                    <div class="loyalty-line earning-highlight">
                        <span class="loyalty-label">⭐ Points You'll Earn:</span>
                        <span class="loyalty-value points-earning" id="points-to-earn">-</span>
                    </div>
                    <div class="loyalty-line">
                        <span class="loyalty-label">Points After Booking:</span>
                        <span class="loyalty-value" id="points-after-booking">-</span>
                    </div>
                </div>
                <div class="loyalty-benefits">
                    <small>💡 ${this.getMembershipBenefit()}</small>
                </div>
            </div>
        `;
    }
    ...
    getMembershipBenefit() {
        ...
        const benefits = {
            ...
            'Silver': 'Earn 1.25 points per $1 spent + priority support',
            'Gold': 'Earn 1.5 points per $1 spent + free upgrades',
            'Platinum': 'Earn 2 points per $1 spent + VIP benefits'
        };
        
        return benefits[this.loyaltyAccount.membershipLevel] || benefits['Bronze'];
    }
    ...
    async calculatePrice() {
        ...
        try {
            ...
            if (this.loyaltyAccount && this.loyaltyService) {
                this.updateLoyaltyEarnings(calculation.totalAmount);
            }
    ...
    updateLoyaltyEarnings(totalAmount) {
        if (!this.loyaltyAccount || !this.loyaltyService) return;

        const pointsToEarn = this.loyaltyService.calculatePointsForBooking(
            totalAmount, 
            this.loyaltyAccount.membershipLevel
        );
        
        const pointsAfterBooking = this.loyaltyAccount.pointsBalance + pointsToEarn;

        const pointsToEarnElement = document.getElementById('points-to-earn');
        const pointsAfterElement = document.getElementById('points-after-booking');

        if (pointsToEarnElement) {
            pointsToEarnElement.textContent = `+${this.formatPoints(pointsToEarn)}`;
            pointsToEarnElement.classList.add('points-highlight');
        }

        if (pointsAfterElement) {
            pointsAfterElement.textContent = this.formatPoints(pointsAfterBooking);
        }

        // Check for potential level upgrade
        this.checkPotentialLevelUpgrade(this.loyaltyAccount.totalPointsEarned + pointsToEarn);
    }

    checkPotentialLevelUpgrade(newTotalPoints) {
        ...
        if (!nextLevelInfo.isMaxLevel && nextLevelInfo.pointsNeeded <= 0) {
            // User will level up with this booking!
            this.showLevelUpgradeNotification(nextLevelInfo.nextLevel);
        }
    }

    showLevelUpgradeNotification(newLevel) {
        const loyaltyEarnings = this.container.querySelector('.loyalty-earnings');
        if (loyaltyEarnings) {
            const upgradeNotification = document.createElement('div');
            upgradeNotification.className = 'level-upgrade-notification';
            upgradeNotification.innerHTML = `
                <div class="upgrade-message">
                    🎉 <strong>Congratulations!</strong> This booking will upgrade you to <strong>${newLevel}</strong> membership!
                </div>
            `;
            loyaltyEarnings.appendChild(upgradeNotification);
        }
    }
    ...
    async handleSubmit(event) {
        ...
        try {
            ...
            // Set loyalty service in booking service if available
            if (this.loyaltyService) {
                this.bookingService.setLoyaltyService(this.loyaltyService);
            }
            ...
            // Show loyalty points earned notification if applicable
            if (this.loyaltyAccount && this.loyaltyService && this.priceCalculation) {
                const pointsEarned = this.loyaltyService.calculatePointsForBooking(
                    this.priceCalculation.totalAmount,
                    this.loyaltyAccount.membershipLevel
                );
                
                this.showLoyaltyEarnedNotification(pointsEarned);
            }
    ...
    showLoyaltyEarnedNotification(pointsEarned) {
        const notification = document.createElement('div');
        notification.className = 'loyalty-earned-notification';
        notification.innerHTML = `
            <div class="notification-content">
                ⭐ <strong>Congratulations!</strong> You earned ${this.formatPoints(pointsEarned)} loyalty points from this booking!
            </div>
        `;
        
        const formMessage = document.getElementById('form-message');
        if (formMessage) {
            formMessage.appendChild(notification);
        }
    }

    // Validation and utility methods (same as original BookingFormComponent)
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = EnhancedBookingFormComponent;
} else {
    window.EnhancedBookingFormComponent = EnhancedBookingFormComponent;
}
